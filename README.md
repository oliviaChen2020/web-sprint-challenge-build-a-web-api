...
1. The core features of Node.js and Express and why they are useful.
   Node is a platform used to execute JavaScript applications outside the browser and allow JavaScript developers use JavaScript to write command line utilities, native programs that run on different operating systems, networking software, and more. The main feature of Express id Middleware functions which can get the request and response objects, operate on them and trigger some actions. And also Routing is one of the main features of Express, we can use routing to map incoming requests to the appropriate request handler base on the URL and HTTP Method used.

1. Understand and explain the use of Middleware.
   Express middleware are functions that execute during the lifecycle of a request to the Express server. Each middleware has access to the HTTP request and response for each route it’s attached to. We use middleware functions to execute codes, make changes to the request and the response objects, end the request-response cycle and call the next middleware in the stack.

1. The basic principles of the REST architectural style.
   The REST architectural style is a set of principles for designing and distributing software which allow us to build scalable client or server side web application. There are six different constraints of REST architecture style, first is that everything is client-server based; client-server communication must be stateless; items are cacheable; uniform interface, each resource should be accessible through a single URL; layered system, and code-on-demand.

1. Understand and explain the use of Express Routers.
   Express Routers is used to organize Web API code, and helps us to create router handlers to provide routing to our app and extending the routing to handle validation, handle 404 or other errors.

1. Describe tooling used to manually test the correctness of an API.
   We use the popular API client called Postman for a software developer to dissect RESTful APIs and test an API’s functionality.
